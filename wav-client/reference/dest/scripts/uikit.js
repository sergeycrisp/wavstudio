(function (global, factory) {
  ...
  function isNode(obj) {
    return (
      toString.call(obj) === "[object Node]" ||
      (isObject(obj) && obj.nodeType === 1)
    );
  }
  ...
  function query(selector, context) {
    return (
      toNode(selector) ||
      find(selector, isContextSelector(selector) ? context : document)
    );
  }
  function queryAll(selector, context) {
    ...
    return (
      (nodes.length && nodes) ||
      findAll(selector, isContextSelector(selector) ? context : document)
    );
  }
    ...
    return new Promise(function (resolve, reject) {
      ...
      on(xhr, "load", function () {
        if (
          ...
          reject(
            assign(Error(xhr.statusText), { xhr: xhr, status: xhr.status })
          );
        }
      });
  ...
  var strats = {};
  strats.args = strats.events = strats.init = strats.created = strats.beforeConnect = strats.connected = strats.ready = strats.beforeDisconnect = strats.disconnected = strats.destroy = function (
    parentVal,
    childVal
  ) {
    ...
    methods: {
      ...
      _toggleHeight: function (el, show) {
        ...
        return (show
          ? Transition.start(
              el,
              assign({}, this.initProps, {
                overflow: "hidden",
                height: endHeight,
              }),
              Math.round(this.duration * (1 - currentHeight / endHeight)),
              this.transition
            )
          : Transition.start(
              el,
              this.hideProps,
              Math.round(this.duration * (currentHeight / endHeight)),
              this.transition
            ).then(function () {
  ...
  var Accordion = {
    ...
    update: function () {
      ...
      this.items.forEach(function (el) {
        return this$1._toggleImmediate(
          $(this$1.content, el),
  ...
  var Video = {
    ...
    data: { automute: false, autoplay: true },
    computed: {
      inView: function (ref) {
  ...
  var HeightMatch = {
    ...
    update: {
      read: function () {
        var this$1 = this;
        css(this.elements, { minHeight: "", boxSizing: "" });
        return {
          rows: !this.row
  ...
  var HeightViewport = {
    ...
    update: {
      ...
      write: function (ref) {
        ...
        var height$$1;
        if (
          (height$$1 = Math.round(toFloat(css(this.$el, "minHeight")))) >=
          offsetHeight(this.$el)
        ) {
  ...
  var SVG = {
    ...
    data: { ratio: 1, id: false, exclude: ["ratio", "src", "icon"], class: "" },
    connected: function () {
      var this$1 = this;
      var assign$$1;
      this.class += " uk-svg";
  ...
  var IconComponent = {
    extends: Icon,
    data: function (vm) {
      return { icon: hyphenate(vm.constructor.options.name) };
    },
  };
  var Slidenav = {
  ...
  function install$1(UIkit) {
    ...
    UIkit.modal.labels = { ok: "Ok", cancel: "Cancel" };
  }
  var Nav = {
    extends: Accordion,
  ...
  var OverflowAuto = {
    mixins: [Class],
    props: { selModal: String, selPanel: String },
    data: { selModal: ".uk-modal", selPanel: ".uk-modal-dialog" },
    computed: {
    ...
    update: {
      read: function () {
        ...
        return {
          ...
          max: Math.max(
            150,
            height(this.modal) - (offset(this.panel).height - height(this.$el))
          ),
        };
      },
      write: function (ref) {
  ...
  var Responsive = {
    ...
    update: {
      ...
      write: function (dim) {
        height(
          this.$el,
          Dimensions.contain({ height: this.height, width: this.width }, dim)
            .height
        );
      },
      events: ["load", "resize"],
        ...
        write: function (ref) {
          ...
          css(
            ...
            assign(
              {
                height:
                  css(this.$el, "position") !== "absolute" ? height$$1 : "",
              },
              css(this.$el, [
                "marginTop",
                "marginBottom",
  ...
  function matchFilter(el, attr$$1, ref) {
    ...
    if (order === void 0) order = "asc";
    return Boolean(
      ((filter$$1 || isUndefined(sort)) &&
        group in stateFilter &&
        (filter$$1 === stateFilter[group] ||
          (isUndefined(filter$$1) && !stateFilter[group]))) ||
  ...
  var SliderAutoplay = {
    ...
    data: { autoplay: false, autoplayInterval: 7000, pauseOnHover: true },
    connected: function () {
      this.startAutoplay();
    },
    disconnected: function () {
    ...
    events: [
      ...
      { name: pointerDown, handler: "stopAutoplay" },
      {
        name: "mouseenter",
        filter: function () {
  ...
  var SliderDrag = {
    data: { threshold: 10, preventCatch: false },
    init: function () {
      var this$1 = this;
  ...
  var LightboxPanel = {
    ...
    events: [
      ...
      { name: "shown", self: true, handler: "showControls" },
      {
        name: "hide",
        self: true,
        handler: function () {
  ...
  var Parallax$1 = {
    ...
    data: { target: false, viewport: 1, easing: 1 },
    computed: {
      target: function (ref, $el) {
        var target = ref.target;
      ...
      getActives: function () {
        ...
        return sortBy(
          slides(list).filter(function (slide) {
            ...
            return (
              slideLeft >= left &&
              slideLeft + slide.offsetWidth <= list.offsetWidth + left
            );
          }),
          "offsetLeft"
        );
  ...
  function getElLeft(el, list) {
    return (
      (el.offsetLeft + (isRtl ? el.offsetWidth - list.offsetWidth : 0)) *
      (isRtl ? -1 : 1)
    );
  }
    ...
    computed: {
      ...
      finite: function (ref) {
        ...
        return (
          finite ||
          getWidth(this.list) <
            this.list.offsetWidth + getMaxWidth(this.list) + this.center
        );
      },
      maxIndex: function () {
        var this$1 = this;
  ...
  var Tooltip = {
    ...
    events:
      ((obj$2 = {}),
      (obj$2["focus " + pointerEnter + " " + pointerDown] = function (e) {
        if (e.type !== pointerDown || !isTouch(e)) {
          this.show();
        }
      }),
      (obj$2.blur = "hide"),
      (obj$2[pointerLeave] = function (e) {
        if (!isTouch(e)) {
          this.hide();
        }
      }),
